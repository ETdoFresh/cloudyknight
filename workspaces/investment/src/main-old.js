...
function calculateInvestorProfit(name) {
  ...
  // Use initial investment as the base if available
  if (investor.initialInvestment && investor.initialInvestment > 0) {
    totalInvested = investor.initialInvestment;
    currentShares = investor.percentage;
  }
  
  // Add any secondary market transactions
  ...
  } else if (!investor.initialInvestment) {
    // Fallback for old data without initialInvestment
    totalInvested = investor.investment || 0;
    currentShares = investor.percentage;
  }
  ...
  // Calculate ROI based on initial investment or net invested
  const baseInvestment = investor.initialInvestment || netInvested;
  ...
  return {
    ...
    totalSold: totalSold,
    initialInvestment: investor.initialInvestment || 0
  };
}
...
function updateShareholdersList() {
  ...
  Object.entries(state.shareholders).forEach(([name, data]) => {
    ...
    // Use initialInvestment if available, otherwise use the calculated investment
    const displayInvestment = data.initialInvestment || stats.totalInvested || stats.invested;

    const row = document.createElement('tr');
    row.innerHTML = `
      <td>${name}</td>
      <td>${data.percentage.toFixed(1)}%</td>
      <td>${formatCurrency(Math.abs(displayInvestment))}</td>
...
function updateInvestorView() {
  ...
  // Display initial investment as positive value
  const displayInvestment = investor.initialInvestment || stats.totalInvested || stats.invested;

  document.getElementById('personal-invested').textContent = formatCurrency(Math.abs(displayInvestment));
